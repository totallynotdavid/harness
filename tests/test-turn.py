...
shutil.copy(
    os.path.join(BIN, "..", "config", "captain.conf"), os.path.join(scratch, "config")
)
...
def claude_msg(mid, blocks, usage=None, ts="2026-09-13T10:00:05Z"):
    return {
        "type": "assistant",
        "timestamp": ts,
        "message": {"id": mid, "content": blocks, "usage": usage or {}},
    }
...
# Prefer the turn payload's answer when it has text.
check(
    "stop answer",
    caplib.turn_answer(
        "claude", {"hook_event_name": "Stop", "last_assistant_message": "PONG"}
    ),
    "PONG",
)

# A tool-ending turn uses the last assistant text from the same turn.
...
check(
    "tool-call fallback",
    caplib.turn_answer("claude", {"last_assistant_message": "", "transcript_path": t}),
    "the real second answer",
)
jsonl(
    t,
    [
...
check(
    "no text this turn",
    caplib.turn_answer(
        "claude", {"last_assistant_message": None, "transcript_path": t}
    ),
    "",
)
...
check(
    "codex fallback",
    caplib.turn_answer("codex", {"last_assistant_message": "", "transcript_path": c}),
    "codex answer",
)

# An account rejection is a session limit. Other StopFailure events are
# harness errors. A plain Stop has no failure.
check(
    "rate_limit",
    raises(
        lambda: caplib.classify(
...
# Token counts include each message once and exclude earlier messages.
jsonl(
    t,
    [
        claude_msg(
...
check(
    "claude tokens",
    caplib.claude_tokens(t, since),
    {"input": 11, "output": 7, "cache_creation": 7, "cache_read": 100},
)

# A failed codex turn has no Stop hook. Its rollout reports the failure once.
jsonl(
    c,
    [
...
session = caplib.Session(
    harness="codex", pane="-", tab="-", turn_dir=turn_dir, label="lint"
)
...
# A codex session that exits after a failed turn reports that failure.
...
session = caplib.Session(
    harness="codex", pane="-", tab="-", turn_dir=exit_dir, label="lint"
)
check(
    "codex error then exit",
    raises(lambda: caplib.wait_turn(session, max_wait=5)),
    "TurnFailed",
)

# wait_turn reads a turn that finishes while launch() is still returning.
...
check(
    "fast turn",
    (raises(lambda: caplib.wait_turn(fast, max_wait=3)), fast.session_id),
    (None, "fast"),
)
os.environ["PATH"] = saved_path

# herdr's done is idle until a client looks at it. An idle session with no turn
# file is reported rather than waited out.
...
check(
    "done without a turn",
    raises(lambda: caplib.wait_turn(quiet, max_wait=30)),
    "TurnLost",
)
os.environ["PATH"] = saved_path
...
for command in (caplib.TURN_COMMAND, caplib.START_COMMAND):
    hook_env = dict(
        os.environ,
        CAP_SESSION_HOOK=os.path.join(spaced, "session-event.sh"),
        CAP_TURN_DIR=event_dir,
    )
    subprocess.run(
        ["sh", "-c", command],
        input=b'{"session_id": "spaced"}',
        env=hook_env,
        check=False,
    )
check(
    "spaced hook path",
    (
        len(caplib.turn_files(event_dir)),
        os.path.exists(os.path.join(event_dir, "session.start")),
    ),
    (1, True),
)

# Captain seeds codex hook trust with the hash codex records.
...
check(
    "codex trust hash",
    caplib.codex_hook_hash("stop", observed),
    "sha256:2719a803c368f17657a0bccae53c32e931ce76cc0f432997511599a43fbd0e46",
)

# A session limit blocks the profile and leaves a resumable record.
    ...
    json.dump(
        {
            "at": caplib.now(),
            "ctx_pct": 12,
            "five_hour": {"pct": 100, "resets_at": caplib.now() + 600},
        },
        fh,
    )
check(
    "limit raises",
    raises(lambda: caplib.limit_reached(profile, session, "k1", None, None, "429")),
    "CapError",
)
check("limit blocks", caplib.lib_ok("profile_blocked", "haiku"), True)
check("limit resumable", caplib.pending_resume(profile, "k1")[0], "s-1")

# Two reviewers with the same prompt on the same profile are separate calls.
...
check(
    "reviewer keys",
    (
        caplib.pending_resume(profile, key_a)[0],
        caplib.pending_resume(profile, key_b)[0],
    ),
    ("s-1", None),
)
...
if failures:
    ...
    sys.exit(1)
print(
    "test-turn: turn payloads, fallbacks, failures and token counts read as what they are"
)
