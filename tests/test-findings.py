...
def case(name, text, *, accepted, verdict=None, problem=None):
    ...
    if not accepted:
        if not problems:
            ...
            failures.append(
                f"{name}: refused for {problems}, want a reason mentioning {problem!r}"
            )
        return
            ...
            "findings": [
                {
                    ...
                    "claim": "prints one where the caller parses two",
                }
            ],
            "checked": checked_all[:1],
        }
    ),
    accepted=True,
    verdict="FAIL",
            ...
            "findings": [
                {
                    ...
                    "severity": "blocker",
                    "confidence": "confirmed",
                    "claim": "the script exits before it writes anything",
                }
            ],
            "checked": checked_all,
        }
            ...
            "findings": [
                {
                    ...
                    "severity": "minor",
                    "confidence": "confirmed",
                    "claim": "readme is a single lowercase word",
                }
            ],
            "checked": checked_all,
        }
...
case(
    ...
    block({"findings": [], "checked": []}),
    accepted=False,
    problem="neither findings nor checked",
)
case(
    "partial coverage",
    block({"findings": [], "checked": checked_all[:1]}),
    accepted=False,
    problem="bin/tool.sh",
)
case(
    ...
    block(
        {
            "findings": [],
            "checked": [
                {"file": "README.md", "note": "ok"},
                {"file": "bin/tool.sh", "note": "fine"},
            ],
        }
    ),
    accepted=False,
            ...
            "findings": [
                {
                    ...
                    "severity": "major",
                    "confidence": "confirmed",
                    "claim": "this file does not exist here",
                }
            ],
            "checked": checked_all,
        }
            ...
            "findings": [
                {
                    ...
                    "severity": "major",
                    "confidence": "confirmed",
                    "claim": "a line the file does not have",
                }
            ],
            "checked": checked_all,
        }
            ...
            "findings": [
                {
                    ...
                    "severity": "critical",
                    "confidence": "confirmed",
                    "claim": "severity outside the scale",
                }
            ],
            "checked": checked_all,
        }
            ...
            "findings": [
                {
                    ...
                    "claim": "the command may fail in an unavailable environment",
                }
            ],
            "checked": checked_all[:1],
        }
...
spec = importlib.util.spec_from_loader(
    "cap_ledger",
    importlib.machinery.SourceFileLoader("cap_ledger", os.path.join(BIN, "cap-ledger")),
)
...
def round_(label, verdict, files, checked, findings=()):
    return {
        ...
        "verdict": verdict,
        "files": files,
        "checked": [{"file": f, "note": "n n n"} for f in checked],
...
checks = [
    (
        "same bytes",
        [passed, round_("A", "FAIL", {"bin/tool.sh": "h1"}, [], [major])],
        1,
    ),
    (
        "changed bytes",
        [passed, round_("A", "FAIL", {"bin/tool.sh": "h2"}, [], [major])],
        0,
    ),
    (
        "minor only",
        [passed, round_("A", "PASS", {"bin/tool.sh": "h1"}, [], [minor])],
        0,
    ),
    (
        "not covered",
        [
            round_("B", "PASS", {"bin/tool.sh": "h1"}, ["README.md"]),
            round_("A", "FAIL", {"bin/tool.sh": "h1"}, [], [major]),
        ],
        0,
    ),
    (
        "fail before pass",
        [round_("A", "FAIL", {"bin/tool.sh": "h1"}, [], [major]), passed],
        0,
    ),
]
...
if failures:
    ...
    sys.exit(1)
print(
    "test-findings: reports are accepted only with coverage, and verdicts follow findings"
)
