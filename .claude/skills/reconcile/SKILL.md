---
name: reconcile
description: Start-of-day orientation - read what is actually open across every project and the live agents, then propose what to work on. Use when the captain asks where things stand, what is pending, or what to do next.
---
...
```sh
cap reconcile           # local: dirty trees, unpushed commits, stashes, stray worktrees, agents
...
3. **Open**: undelivered work, ranked by what it is costing to leave open. Not an inventory
...
So is a branch that was never delivered and a PR nobody reviewed. Say so.
